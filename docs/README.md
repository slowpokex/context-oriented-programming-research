# Documentation

Welcome to the Context-Oriented Programming (COP) research documentation. This directory contains comprehensive research, specifications, and technical details about COP as an emerging programming paradigm.

## Quick Navigation

| Section | Description | Start Here |
|---------|-------------|------------|
| [Research](./research/) | Core research findings, analysis, and opinions | [Key Findings](./research/key-findings.md) |
| [Specification](./specification/) | Package format spec and architecture diagrams | [Package Format](./specification/package-format.md) |
| [Build](./build/) | Deep-dive into the "build" concept in COP | [Build Concept](./build/concept.md) |

## Document Index

### Research (`docs/research/`)

| Document | Description |
|----------|-------------|
| [key-findings.md](./research/key-findings.md) | Distilled insights and summary — **start here for a quick overview** |
| [main-research.md](./research/main-research.md) | Comprehensive research report with detailed analysis |
| [deep-analysis.md](./research/deep-analysis.md) | Philosophical and technical deep-dive into COP |
| [tool-comparison.md](./research/tool-comparison.md) | Analysis of existing LLM tools (LangChain, PromptFlow, etc.) |
| [opinion.md](./research/opinion.md) | Perspective on COP as a paradigm shift |

### Specification (`docs/specification/`)

| Document | Description |
|----------|-------------|
| [package-format.md](./specification/package-format.md) | Draft COP package format specification (`.cop`) |
| [architecture.md](./specification/architecture.md) | System architecture diagrams for the COP ecosystem |

### Build (`docs/build/`)

| Document | Description |
|----------|-------------|
| [concept.md](./build/concept.md) | What "build" means in COP — context assembly, evaluation, transformation |
| [internals.md](./build/internals.md) | Technical implementation details, algorithms, and data structures |

## Reading Order

**For newcomers**, we recommend this reading order:

1. **[Key Findings](./research/key-findings.md)** — Quick summary of what COP is and why it matters
2. **[Main Research](./research/main-research.md)** — Comprehensive research findings
3. **[Build Concept](./build/concept.md)** — Understanding what "build" means in COP
4. **[Package Format](./specification/package-format.md)** — The proposed specification
5. **[Tool Comparison](./research/tool-comparison.md)** — How existing tools relate to COP

**For deep understanding**, add:

6. **[Deep Analysis](./research/deep-analysis.md)** — Philosophical foundations
7. **[Build Internals](./build/internals.md)** — Implementation details
8. **[Architecture](./specification/architecture.md)** — System diagrams
9. **[Opinion](./research/opinion.md)** — Perspective and predictions

## Related Resources

- [Example COP Package](../examples/customer-support-agent/) — A complete example implementation
- [Main README](../README.md) — Project overview
- [AGENTS.md](../AGENTS.md) — Instructions for AI agents working with this repository
